...
import authorsPageInitialPathsE5c0eResource from '../../resources/authors-page-initial-paths-e5c0e'
import authorsPageInitialProps57c46Resource from '../../resources/authors-page-initial-props-57c46'
  ...
  return (
    <>
      <div className="authors11-container">
        <Head>
          <title>Authors1 - Customer Group Engineer</title>
          <meta
            property="og:title"
            content="Authors1 - Customer Group Engineer"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPathsE5c0eResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialProps57c46Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
